"""Classic stop-and-wait Zmodem-like protocol (no sliding window).

This is the original simple protocol that acknowledges every chunk
before sending the next one.  It is more predictable on half-duplex
mesh radio where the sliding-window approach can cause collisions.

The public API is compatible with the sliding-window ``zmodem`` module:
``Sender`` and ``Receiver`` expose the same methods so the surrounding
application code can use either module interchangeably.
"""
...
class Sender:
    def __init__(self, fobj, chunk_size: int = 256, window_size: int = 1):
        # window_size accepted for API compatibility but ignored (always 1)
        self.fobj = fobj
...
class Receiver:
    def __init__(self, fobj_or_path, ack_interval=1):
        ...
        *ack_interval* is accepted for API compatibility but ignored:
        classic mode ACKs every chunk.
